...
gen_network_configuration()
{
    ...
    for interface in $CONTEXT_INTERFACES; do
        ...
        sed -i'' "s/send host-name.*/send host-name \"$SET_HOSTNAME\";/g" /etc/dhclient.conf
        sed -i'' "s/send dhcp-requested-address.*/send dhcp-requested-address $IP;/g" /etc/dhclient.conf
